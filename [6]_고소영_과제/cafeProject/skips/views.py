...
def skip_update_comment(request, skip_id, com_id):
    comment = SkipComment.objects.get(id=com_id)
...
def skip_like(request, id):
    skip = get_object_or_404(PostSkip, pk=id)
    if request.user in skip.likes.all():
        skip.likes.remove(request.user)
    else:
        skip.likes.add(request.user)
    return redirect('skip_detail', id=id)